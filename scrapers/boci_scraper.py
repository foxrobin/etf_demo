"""
BOCI: download holdings from direct file URLs (XLSX or CSV).

Example XLSX:
  https://www.boci-pru.com.hk/doc/Fund_Daily_A_Shares/ETFHoldingsDI_WISE-CSIHK100_14042026%20162100_20260414_172342.xlsx
"""
...
def _etf_code_from_boci_path(url: str) -> Optional[str]:
    """e.g. .../ETFHoldingsDI_WISE-CSIHK100_...xlsx -> WISE-CSIHK100"""
    path = unquote(urlparse(url).path)
    m = re.search(r"ETFHoldingsDI_([^_/]+)_", path, re.I)
...
def _as_of_from_boci_cells(rows: List[List[str]]) -> Optional[str]:
...
def _is_holdings_header_row(cells: List[str]) -> bool:
    if not cells or not cells[0]:
        return False
    first = cells[0].lower()
    return "stock code" in first or "股份代號" in cells[0]
...
def _parse_holdings_table(rows: List[List[str]]) -> tuple[List[List[str]], Optional[str]]:
    as_of = _as_of_from_boci_cells(rows)
    ...
    if header_idx is None:
        raise ValueError("Cannot find holdings header row (Stock Code / 股份代號).")
...
class BOCIScraper:
    """Download BOCI daily holdings XLSX (or CSV) and normalize to table rows."""
    ...
    def scrape_url(self, url: str) -> PageResult:
        etf_code = _etf_code_from_boci_path(url)
        try:
